	...
	class Awl_Portfolio_Filter_Gallery {
		...
		public function pfg_menu() {
			...
			if($old_setting_update_status == 1 && $pfg_hide_notice == 1) {
				$update_old_settings_menu = add_submenu_page( 'edit.php?post_type='.PFG_PLUGIN_SLUG, __( 'Update Old Settings', 'portfolio-filter-gallery' ), __( 'Update Old Settings', 'portfolio-filter-gallery' ), 'manage_options', 'pfg-update-plugin', array( $this, 'update_old_settings_page') );
			}
		...
		public function admin_add_meta_box() {
			...
			add_meta_box( __('pfg-youtube', 'portfolio-filter-gallery'), __('Subscribe', 'portfolio-filter-gallery'), array(&$this, 'PFG_Youtube'), 'awl_filter_gallery', 'side', 'default' );
		}
		// meta upgrade pro
		public function pfg_upgrade_pro() { ?>
			...
			<a href="http://awplife.com/demo/portfolio-filter-gallery-premium/" target="_new" class="pfg-btn button button-primary" style="background: #496481; text-shadow: none;"><span class="dashicons dashicons-search" style="line-height:1.4;" ></span> <?php _e('Live Demo', 'portfolio-filter-gallery'); ?></a>
			<a href="http://awplife.com/account/signup/portfolio-filter-gallery" target="_new" class="pfg-btn button button-primary" style="background: #496481; text-shadow: none;"><span class="dashicons dashicons-unlock" style="line-height:1.4;" ></span> <?php _e('Upgrade To Pro', 'portfolio-filter-gallery'); ?></a>
		<?php }
		// meta upgrade pro
		public function PFG_Youtube() { ?>
			<a href="https://www.youtube.com/c/AWPLife-WordPress-Themes-Plugins" target="_new"><img src="<?php echo PFG_PLUGIN_URL ?>img/Portfolio Side Image youtube.jpg"/ width="250" height="280"></a>
		<?php }
		// meta rate us
		...
		<br>
		<div style="text-align:center">
			<a href="https://wordpress.org/support/plugin/portfolio-filter-gallery/reviews/" target="_new" class="pfg-btn button button-primary button-large" style="background: #496481; text-shadow: none;"><span class="dashicons dashicons-heart" style="line-height:1.4;" ></span> <?php _e('Please Rate Us', 'portfolio-filter-gallery'); ?></a>
		</div>	
		<?php }
		...
		public function _pfg_ajax_callback_function($id) {
			if ( current_user_can( 'manage_options' ) ) {
				...
				<li class="item image">
					...
					<input type="text" name="image-title[]"  style="width: 98%;" placeholder="Image Title" value="<?php echo get_the_title($id); ?>">
					...
					?>
					<select class="pfg-filters form-control" name="filters[<?php echo esc_attr($id); ?>][]" multiple="multiple" style="width: 98%;">
						<?php
						foreach ($all_category as $key => $value) {
							if($key != 0) {
								?><strong><option value="<?php echo esc_attr($key); ?>"><?php echo stripslashes(esc_html($value)); ?></option></strong><?php
							}
						}
						?>
					</select>
